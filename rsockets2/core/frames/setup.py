...
class SetupFrame(FrameHeader):
    ...
    def metadata(buffer: bytes) -> memoryview:
        ...
        return memoryview(buffer)[start:(start + length)]

    @staticmethod
    def data_start(buffer: bytes) -> int:
    ...
    def data(buffer: bytes) -> memoryview:
        return memoryview(buffer)[SetupFrame.data_start(buffer):]
    ...
    def create_new(
        ...
        SetupFrame.encode_frame_header(
            buffer, 0, 0x01, False, metadata != None)
